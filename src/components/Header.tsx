...
const Header = () => {
  ...
  const activeSearchScopes = searchScopes.filter((scope) => {
    if (scope.key === "products" && !isShoppingEnabled) return false;
    if (scope.key === "tours" && !isToursEnabled) return false;
    if (scope.key === "training" && !isTrainingEnabled) return false;
    return true;
  });

  // If the current scope is disabled, fallback to the first active scope
  const activeScopeKey = activeSearchScopes.find(s => s.key === searchScope) ? searchScope : (activeSearchScopes[0]?.key || "products");
  const currentScope = activeSearchScopes.find((s) => s.key === activeScopeKey) || searchScopes[0];

  // Dynamically filter nav links
  ...
  const { data: categoriesData, isLoading: isLoadingCats } = useQuery({
    ...
    enabled: catOpen && isShoppingEnabled,
  });
  ...
  const handleHeaderSearch = () => {
    ...
    if (!q || activeSearchScopes.length === 0) return;
    let target = "/shop";
    if (activeScopeKey === "tours") target = "/tours";
    if (activeScopeKey === "training") target = "/education";
              ...
              <div className="flex w-full border border-border rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-primary/30">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start" className="w-44">
                    {activeSearchScopes.map((scope) => (
                      <DropdownMenuItem
                        key={scope.key}
                        onClick={() => setSearchScope(scope.key)}
                        className={`gap-2.5 ${activeScopeKey === scope.key ? "bg-primary/10 text-primary font-medium" : ""}`}
                      >
